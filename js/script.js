...
//	console.log(inputFive)
	...
	sealedCash.addEventListener("input", e => {
		...
		amountWords.innerText = convertValue(parseFloat(sumTotal.innerText).toFixed(2))
...
function ggg(){
	window.print();
	 ...
	 function bbb(){
		...
		const now = new Date();
		const nazwaZlecenia = "UTARG WARSZAWA "+now.getDate()+"."+(now.getMonth()+1)+"."+now.getFullYear()
